...
public class ConnectionRequestForServiceDiscoveryTests extends ScenarioTest<GivenStage, WhenStage, ThenStage> {
    ...
    public void retries_caused_by_rejects(int maxRequests, int rejectedRequests, int actualRequests) {
        ...
        when().a_connection_request_for_service_discovery_is_issued();
        then().$_connection_requests_are_issued(actualRequests).
            and().device_status_is(ThingStatus.OFFLINE);
        // @formatter:on
    }
